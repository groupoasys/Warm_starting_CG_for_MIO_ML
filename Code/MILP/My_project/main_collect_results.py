...
def collect_results_active_constraints_general_MILP(number_of_divisions,
    ...
    columns_to_drop = columns_elapsed_time + columns_computational_complexity + columns_objective_value + columns_gap + columns_termination_condition
    ...
    indexes_train = range(0, 200)
    indexes_test = range(0, 200)
    ...
    averaged_performance_results_train.to_csv('00_summary_performance_MILP_train.csv', sep=';')
    averaged_performance_results_test.to_csv('00_summary_performance_MILP_test.csv', sep=';')

    coefficients_and_labels_and_more_info.to_csv('MILP_' + beginning_file_active_constraints + '_and_more_info.csv',
                                                 sep=';')
    coefficients_and_labels_and_more_info.to_csv(
        '../../../Data/' + folder_particular_dataset + '/MILP_' + beginning_file_active_constraints + '_and_more_info.csv',
        sep=';')

    coefficients_and_labels.to_csv('MILP_' + beginning_file_active_constraints + '_info.csv', sep=';')
    coefficients_and_labels.to_csv(
        '../../../Data/' + folder_particular_dataset + '/MILP_' + beginning_file_active_constraints + '_info.csv',
        sep=';')
    return 0
