...
def collect_results_CG_iteratively(number_of_divisions,
                                   database_results_folder='Results',
                                   beginning_file_active_constraints='coefficients_and_labels_iteratively',
    ...
    for division in range(number_of_divisions):
        coefficients_and_labels_per_division = pd.read_csv('performance_results_iteratively_' + str(division) + '.csv',
                                                           sep=';', index_col=0)
